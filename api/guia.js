...
const { handleSearchConsole } = require('../lib/search-console');
...
async function handleChat(req, res) {
  ...
  if (req.method !== 'POST') { res.setHeader('Allow', 'POST, OPTIONS'); return json(res, 405, { error: 'Método não permitido.' }); }
  try {
    ...
    if (openRouterKey) { provider = 'openrouter'; response = await fetch('https://openrouter.ai/api/v1/chat/completions', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${openRouterKey}`, 'HTTP-Referer': 'https://www.maispersuasaomaisvendas.com.br/ai', 'X-Title': 'MPMV AI' }, body: JSON.stringify({ model: process.env.OPENROUTER_MODEL || 'openrouter/free', messages: [{ role: 'system', content: MPMV_INSTRUCTIONS }, ...messages], temperature: 0.7 }) }); }
    else if (openAIKey) { provider = 'openai'; response = await fetch('https://api.openai.com/v1/responses', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${openAIKey}` }, body: JSON.stringify({ model: process.env.OPENAI_MODEL || 'gpt-5.6', instructions: MPMV_INSTRUCTIONS, input: messages }) }); }
    else return json(res, 500, { error: 'A IA não está configurada na Vercel.' });
    const data = await response.json();
    if (!response.ok) { console.error('MPMV AI Provider Error:', data); return json(res, response.status >= 400 && response.status < 600 ? response.status : 502, { error: data?.error?.message || data?.message || 'Falha no provedor de IA.' }); }
  ...
  } catch (error) { console.error('MPMV AI Error:', error); return json(res, 500, { error: error?.message || 'Erro interno ao processar a mensagem.' }); }
}
...
module.exports = async function handler(req, res) {
  ...
  res.setHeader('Cache-Control', 'no-store, max-age=0'); res.setHeader('Location', '/guia-pratico-persuasao-pra-vender-todo-santo-dia.pdf'); return res.status(302).end();
};
