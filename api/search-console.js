...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const pages=(pagesData.rows||[]).map(row).filter(x=>{try{return new URL(x.keys[0]||'').pathname.startsWith('/blog/')}catch(_){return false}}).sort((a,b)=>b.clicks-a.clicks||b.impressions-a.impressions);
    ...
    return json(res,200,{ok:true,siteUrl:SITE_URL,startDate:isoDate(start),endDate:isoDate(end),days,total,articleTotals,pages:pages.slice(0,100),queries:queries.slice(0,100)});
