const SECRET='qa_XZWUut6NnfkSaJQQxtIZzSY4';
...
async function post(path,payload,headers={}){return read(await fetch(BASE+path,{method:'POST',headers:{'content-type':'application/json',...headers},body:JSON.stringify(payload)}))}
async function get(path){return read(await fetch(BASE+path,{headers:{'cache-control':'no-cache'}}))}

module.exports=async function handler(req,res){
  res.setHeader('Cache-Control','no-store');
  if(String(req.query&&req.query.key||'')!==SECRET)return res.status(404).json({ok:false});
  ...
  results.guide=await post('/api/leads',{
    name:'Teste E2E Guia',
    email:`irodrigosd+teste-e2e-guia-${stamp}@${testDomain}`,
    source:'guia-pratico',
    page:'/',
    tracking:{
      landingPage:'/?utm_source=instagram&utm_medium=paid_social&utm_campaign=qa_e2e_guia',
      currentPage:'/',referrer:'https://l.instagram.com/',source:'instagram',medium:'paid_social',campaign:'qa_e2e_guia',
      adset:'qa_adset',ad:'qa_ad',fbclid:'QA_FBCLID_GUIDE',gclid:''
    }
  });
  ...
  results.mentoria=await post('/api/mentoria',{
    name:'Teste E2E Mentoria',email:`irodrigosd+teste-e2e-mentoria-${stamp}@${testDomain}`,phone:'11999999999',
