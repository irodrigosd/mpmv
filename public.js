(function(){
  ...
  function openModal(){
    show(backdrop); if(backdrop)backdrop.setAttribute('aria-hidden','false');
  ...
  document.querySelectorAll('[data-mpmv-cookie]').forEach(function(btn){
    btn.addEventListener('click',function(){
      ...
      hide(cookie);
      if(window.fbq){try{fbq('consent',v==='accepted'?'grant':'revoke');if(v==='accepted')fbq('track','PageView')}catch(e){}}
    });
  });

  function startDownload(){
    var a=document.createElement('a');
    a.href='/assets/downloads/guia-pratico-persuasao-pra-vender-todo-santo-dia.pdf';
  ...
  if(form)form.addEventListener('submit',async function(e){
    ...
    try{
      ...
      if(errorEl){
        errorEl.textContent=err.status===503
          ? 'A integração com o Brevo ainda não foi configurada na Vercel.'
          : 'Não consegui registrar seu e-mail agora. Tente novamente.';
        show(errorEl);
      }
    }finally{
