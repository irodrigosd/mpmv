...
async function createCampaign(body,mode){
  const name=clean(body.name,120),subject=clean(body.subject,180),preheader=clean(body.preheader,180),message=clean(body.body,20000),ctaLabel=clean(body.ctaLabel,80)||'Saiba mais';
  ...
  const payload={name:brevoName,subject,previewText:preheader,htmlContent:campaignHtml({preheader,body:message,ctaLabel,ctaUrl,campaignSlug,testMode:mode==='test',showCtaButton}),sender:await activeSender(),recipients:{listIds:[targetListId]},replyTo:process.env.MPMV_REPLY_TO||undefined};
