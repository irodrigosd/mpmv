...
const NAME_ATTRIBUTE = 'MPMV_NOME';
...
function escapeHtml(v){return String(v==null?'':v).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#039;'}[m]));}
...
function nameOf(c){const a=c&&c.attributes||{};return clean(a.NOME||a.NAME||a.FIRSTNAME||a.FIRST_NAME||'',120)||'Olá';}
function personalize(text,testName=''){
  const replacement=testName||`{{ contact.${NAME_ATTRIBUTE} }}`;
  return String(text||'').replace(/\{\{\s*nome\s*\}\}/gi,replacement);
}
function normalizeLooseLinks(text){
  let source=String(text||'').replace(/\r\n/g,'\n');
  source=source.replace(/\[\[([^\]\n]{1,240})\]\]\s*\(\s*(https?:\/\/[^\s)]+)\s*\)/g,'[$1]($2)');
  source=source.replace(/\[\[([^\]\n]{1,240})\]\]\s*\n\s*\(?(https?:\/\/[^\s)]+)\)?/g,'[$1]($2)');
  source=source.replace(/\[([^\]\n]{1,240})\]\s*\n\s*\(?(https?:\/\/[^\s)]+)\)?/g,'[$1]($2)');
  ...
  for(const rawLine of lines){
    ...
    if(onlyUrl&&out.length&&out[out.length-1].trim()){
      ...
      const prevTrim=prev.trim().replace(/^\[+|\]+$/g,'');
      const alreadyLinked=/\[[^\]]+\]\s*\(\s*https?:\/\//i.test(prev);
...
function formatInline(text,campaignSlug,testMode){
  ...
  while((m=re.exec(source))){
    ...
    html+=`<a href="${escapeHtml(href)}" style="color:#0d6f9f;text-decoration:underline;font-weight:700">${escapeHtml(label)}</a>`;
    last=re.lastIndex;
  }
  html+=escapeHtml(source.slice(last));
  return html.replace(/\n/g,'<br>');
}
function paragraphs(text,campaignSlug,testMode,testName=''){const normalized=normalizeLooseLinks(personalize(clean(text,20000),testName));return normalized.split(/\n{2,}/).map(p=>p.trim()).filter(Boolean).map(p=>`<p style="margin:0 0 18px;line-height:1.65;color:#17202a;font-size:16px">${formatInline(p,campaignSlug,testMode)}</p>`).join('');}
...
function campaignHtml({preheader,body,ctaLabel,ctaUrl,campaignSlug,testMode=false,showCtaButton=true,testName=''}){
  const parts=splitSignature(body);
  const mainHtml=paragraphs(parts.main,campaignSlug,testMode,testName);
  const signatureHtml=parts.signature?paragraphs(parts.signature,campaignSlug,testMode,testName):'';
  let ctaHtml='';
  if(showCtaButton){
...
async function ensureCampaignAttributes(){
  const all=await brevo('/contacts/attributes');
  const attrs=Array.isArray(all.attributes)?all.attributes:[];
  const have=new Set(attrs.map(a=>String(a&&a.name||'').toUpperCase()));
  for(const attr of [TRACK_ATTRIBUTE,NAME_ATTRIBUTE]){
    if(have.has(attr))continue;
    try{await brevo('/contacts/attributes/normal/'+attr,{method:'POST',body:JSON.stringify({type:'text'})});}catch(e){if(e.status!==400)throw e;}
  }
}
...
async function prepareTracking(contacts){await ensureCampaignAttributes();const usable=contacts.filter(c=>c&&Number(c.id)>0&&emailOk(c.email)&&!c.emailBlacklisted);for(let i=0;i<usable.length;i+=8){await Promise.all(usable.slice(i,i+8).map(c=>brevo('/contacts/'+encodeURIComponent(String(c.id))+'?identifierType=contact_id',{method:'PUT',body:JSON.stringify({attributes:{[TRACK_ATTRIBUTE]:trackingToken(c.id),[NAME_ATTRIBUTE]:nameOf(c)}})})));}return usable;}
...
async function createCampaign(body,mode){
  const name=clean(body.name,120),rawSubject=clean(body.subject,180),preheader=clean(body.preheader,180),message=clean(body.body,20000),ctaLabel=clean(body.ctaLabel,80)||'Saiba mais';
  const showCtaButton=body.showCtaButton!==false;
  const ctaUrl=showCtaButton?destination(body.ctaUrl):'';
  if(name.length<3||rawSubject.length<3||message.length<10)throw Object.assign(new Error('campaign_fields_required'),{status:400});
  ...
  const testEmail=mode==='test'?clean(body.testEmail,180).toLowerCase():'';
  const testContact=mode==='test'?all.find(c=>String(c.email||'').toLowerCase()===testEmail):null;
  const testName=testContact?nameOf(testContact):'Teste';
  const subject=personalize(rawSubject,mode==='test'?testName:'');
  ...
  const payload={name:brevoName,subject,previewText:personalize(preheader,mode==='test'?testName:''),htmlContent:campaignHtml({preheader,body:message,ctaLabel,ctaUrl,campaignSlug,testMode:mode==='test',showCtaButton,testName:mode==='test'?testName:''}),sender:await activeSender(),recipients:{listIds:[targetListId]},replyTo:process.env.MPMV_REPLY_TO||undefined};
  ...
  if(mode==='test'){if(!emailOk(testEmail))throw Object.assign(new Error('invalid_test_email'),{status:400});await brevo(`/emailCampaigns/${campaignId}/sendTest`,{method:'POST',body:JSON.stringify({emailTo:[testEmail]})});}
