...
async function handleContactsAdmin(req,res){
  ...
  try{
    ...
    if(action==='remove-list'){
      const emails=Array.isArray(body.emails)?body.emails:[body.email];await removeFromList(effectiveList,emails);return json(res,200,{ok:true,removed:emails.length,listId:effectiveList});
    }
    if(action==='delete-contact'){
      await deleteContact(body.email);return json(res,200,{ok:true,deleted:1});
    }
    if(action==='delete-many'){
      const emails=(Array.isArray(body.emails)?body.emails:[]).map(x=>clean(x,180).toLowerCase()).filter(emailOk);let deleted=0,failed=[];for(const email of emails){try{await deleteContact(email);deleted++}catch(e){failed.push({email,error:e.message})}}return json(res,failed.length?207:200,{ok:failed.length===0,deleted,failed});
    }
