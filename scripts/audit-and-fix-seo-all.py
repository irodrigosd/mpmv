...
SEO_TITLE_RE = re.compile(r'<meta\b[^>]*name=["\']mpmv:seo-title["\'][^>]*content=["\']([^"\']*)["\'][^>]*>', re.I)
TITLE_RE = re.compile(r'<title>(.*?)</title>', re.I | re.S)
CANON_RE = re.compile(r'<link\b[^>]*rel=["\']canonical["\'][^>]*>', re.I)
...
def score(src, focus):
    title=text(TITLE_RE.search(src).group(1)) if TITLE_RE.search(src) else ""
    h1=text(H1_RE.search(src).group(1)) if H1_RE.search(src) else ""
    bodym=re.search(r"<article\b[^>]*>(.*?)</article>",src,re.I|re.S); body=bodym.group(1) if bodym else src
    first=" ".join(words(body)[:150]).lower(); fl=focus.lower()
    h2text=" ".join(text(x) for x in re.findall(r"<h[23]\b[^>]*>(.*?)</h[23]>",body,re.I|re.S)).lower()
    dm=DESC_RE.search(src); desc=html.unescape(dm.group(1)) if dm else ""
    sm=SEO_TITLE_RE.search(src); seot=html.unescape(sm.group(1)) if sm else title
...
def fix(src, slug, focus):
    ...
    if not 115<=len(desc)<=160:
        ...
        src=meta_replace(src,"description",desc[:160])
    sm=SEO_TITLE_RE.search(src); seot=html.unescape(sm.group(1)) if sm else ""
    if not 35<=len(seot)<=62:
        seot=f"{focus} | Guia prático | MPMV"
        if len(seot)>62: seot=f"{focus} | MPMV"[:62].rstrip()
        src=meta_replace(src,"mpmv:seo-title",seot)
    m=H1_RE.search(src)
